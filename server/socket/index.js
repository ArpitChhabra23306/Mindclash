    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join_queue', async (data) => {
            try {
                ...
                if (matchmakingQueue.get(queueKey).some(e => e.userId === socket.user._id.toString())) {
                    return socket.emit('error', { message: 'You are already in the matchmaking queue' });
                }
        ...
        socket.on('submit_argument', async (data) => {
            try {
                ...
                // Prevent submissions if both sides are already at the limit
                if (proMessageCount >= MESSAGES_PER_SIDE_TO_END && conMessageCount >= MESSAGES_PER_SIDE_TO_END) {
                    return socket.emit('error', { message: 'This debate is already concluding.' });
                }
                ...
                if (proMessageCount >= MESSAGES_PER_SIDE_TO_END && conMessageCount >= MESSAGES_PER_SIDE_TO_END) {
                    ...
                    await finishDebate(io, debateId);
                } else {
 ...
 */
async function finishDebate(io, debateId) {
    console.log('🏁 finishDebate called for:', debateId);

    // Atomic update to mark as finished immediately - acts as a lock
    const debate = await Debate.findOneAndUpdate(
        { _id: debateId, status: 'active' },
        { 
            status: 'finished', 
            endedAt: new Date(),
            bettingOpen: false 
    ...
    .populate('proTeam.user')
    .populate('conTeam.user');

    if (!debate) {
        console.log('⚠️ finishDebate: Debate already processed or not found');
        return;
    }
